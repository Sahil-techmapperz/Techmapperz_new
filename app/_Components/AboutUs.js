    ...
    <section className="bg-theme-bg border-t border-gray-200">
      <div className='w-full h-full  py-8 max-sm:py-2 max-sm:px-4 '>
        <div className="max-w-7xl mx-auto relative">
          ...
          {/* Center Logo - Displayed on desktop */}
          <div className="hidden lg:flex absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 w-44 h-44 xl:w-56 xl:h-56 rounded-full bg-white shadow-lg items-center justify-center z-10">
            <Image
              src={logo}
              alt="Techmapperz Logo"
              width={120}
              height={120}
              className="object-contain lg:w-[160px] lg:h-[160px] xl:w-[200px] xl:h-[200px]"
            />
          </div>

          {/* Features Grid - Made responsive */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 sm:gap-y-8 md:gap-x-8 lg:gap-x-24 xl:gap-x-[15rem] md:gap-y-[2rem]">
